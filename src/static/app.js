      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = details.participants.length
          ? `<ul>${details.participants
              .map(
                (email) =>
                  `<li><span class="participant-email">${email}</span><button class="participant-remove" data-activity="${name}" data-email="${email}" type="button" aria-label="Remove ${email}">✕</button></li>`
              )
              .join("")}</ul>`
